...
const ipoMasterService = async (params) => {
  ...
  result = await Promise.all(resp.map(async (data) => {
    ...
    data.biddingStartDate = moment(data.biddingStartDate,'DD-MM-YYYY').format("YYYY-MM-DD")
    data.biddingEndDate = moment(data.biddingEndDate,'DD-MM-YYYY').format("YYYY-MM-DD")
...
const updateIPOService = async (params) => {
  let applicationNumber = null;
  let cmsIpoDatesResp = await cmsIpoDates
    .findOne({ ipoisinNumber: params.ipoisinNumber })
    .lean();
...
const cancelIPOService = async (params) => {
  let applicationNumber = null;
  let cmsIpoDatesResp = await cmsIpoDates
    .findOne({ ipoisinNumber: params.ipoisinNumber })
    .lean();
